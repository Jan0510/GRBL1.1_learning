...
#define AMASS_LEVEL1 (F_TIMER/8000) // Over-drives ISR (x2). Defined as F_CPU/(Cutoff frequency in Hz)
#define AMASS_LEVEL2 (F_TIMER/4000) // Over-drives ISR (x4)
#define AMASS_LEVEL3 (F_TIMER/2000) // Over-drives ISR (x8)
...
void st_wake_up() //系统退出休眠状态
{
	// Enable stepper drivers.
	if (bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)) 
	{ 
		GPIO_SetBits(GPIOC,(1<<STEPPERS_DISABLE_BIT));
	}
	else 
	{ 
		GPIO_ResetBits(GPIOC,(1<<STEPPERS_DISABLE_BIT)); 
	}
  ...
  if (sys.state & (STATE_CYCLE | STATE_HOMING)){
    ...
    // Enable Stepper Driver Interrupt
	  TIM_ClearITPendingBit(TIM3,TIM_IT_Update);   //清除定时器3的中断标志位
	  TIM_ITConfig(TIM3,TIM_IT_Update,ENABLE);     //使能定时器3的更新中断
    TIM_Cmd(TIM3,ENABLE);                        //开启定时器3，控制步进脉冲的频率
  }
}
...
void st_go_idle() 
{
  ...
  // Disable Stepper Driver Interrupt. Allow Stepper Port Reset Interrupt to finish, if active.
	TIM_ClearITPendingBit(TIM3,TIM_IT_Update);     //清除定时器3的中断标志位
	TIM_ITConfig(TIM3,TIM_IT_Update,DISABLE);      //定时器3的更新中断去使能
...
// SPINDLE_ENABLE_PORT ^= 1<<SPINDLE_ENABLE_BIT; // Debug: Used to time ISR
  ...
  // Set the direction pins a couple of nanoseconds before we step the steppers
	...
	if(st.dir_outbits & (1<<X_DIRECTION_BIT))
		GPIO_SetBits(GPIOC,(1<<X_DIRECTION_BIT));
	else
		GPIO_ResetBits(GPIOC,(1<<X_DIRECTION_BIT));
	if(st.dir_outbits & (1<<Y_DIRECTION_BIT))
		GPIO_SetBits(GPIOC,(1<<Y_DIRECTION_BIT));
	else
		GPIO_ResetBits(GPIOC,(1<<Y_DIRECTION_BIT));
	if(st.dir_outbits & (1<<Z_DIRECTION_BIT))
		GPIO_SetBits(GPIOC,(1<<Z_DIRECTION_BIT));
	else
		GPIO_ResetBits(GPIOC,(1<<Z_DIRECTION_BIT));
  // Then pulse the stepping pins
	...
	if(st.step_outbits & (1<<X_STEP_BIT))
		GPIO_SetBits(GPIOA,(1<<X_STEP_BIT));
	else
		GPIO_ResetBits(GPIOA,(1<<X_STEP_BIT));
	if(st.step_outbits & (1<<Y_STEP_BIT))
		GPIO_SetBits(GPIOA,(1<<Y_STEP_BIT));
	else
		GPIO_ResetBits(GPIOA,(1<<Y_STEP_BIT));
	if(st.step_outbits & (1<<Z_STEP_BIT))
		GPIO_SetBits(GPIOA,(1<<Z_STEP_BIT));
	else
		GPIO_ResetBits(GPIOA,(1<<Z_STEP_BIT));
  #endif  

  // Enable step pulse reset timer so that The Stepper Port Reset Interrupt can reset the signal after
  // exactly settings.pulse_microseconds microseconds, independent of the main Timer1 prescaler.
	TIM_SetAutoreload(TIM4,st.step_pulse_time);// Reload Timer0 counter
	TIM_PrescalerConfig(TIM4,64-1,TIM_PSCReloadMode_Immediate);
	TIM_Cmd(TIM4,ENABLE);// Begin Timer0. Full speed, 1/8 prescaler
  ...
  if (st.exec_segment == NULL) {
    ...
    if (segment_buffer_head != segment_buffer_tail) {
      ...
      // Initialize step segment timing per step and load number of steps to execute.
		TIM_SetAutoreload(TIM3,st.exec_segment->cycles_per_tick);
...
void stepper_init()
{
  ...
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4 | GPIO_Pin_2|GPIO_Pin_3;//step引脚初始化
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;                    //普通输出模式
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;                   //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//50MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
  GPIO_Init(GPIOA, &GPIO_InitStructure);//脉冲引脚与使能引脚初始化
	
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1|GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_0;//方向引脚和使能引脚初始化
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//普通输出模式
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//50MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
  GPIO_Init(GPIOC, &GPIO_InitStructure);//方向引脚初始化

  // Configure Timer 3: Stepper Driver Interrupt
	TIM3_Int_Init(500-1,1-1);
  
  TIM4_Int_Init(500-1,64-1);
  #ifdef STEP_PULSE_DELAY
...
void st_prep_buffer()
{
	...
	{ 
		...
		if (prep.current_speed == (float)0.0) { return; } // Nothing to do. Bail.
	}
	...
	{ 
      ...
      */
			...
			inv_2_accel = (float)0.5/pl_block->acceleration;
			...
			else  // [Normal Operation]
			{
				...
				intersect_distance =
					(float)0.5*(pl_block->millimeters+inv_2_accel*(pl_block->entry_speed_sqr-exit_speed_sqr));
				if (intersect_distance > (float)0.0) 
				{
					...
					{ 
						...
						{ 
							...
							prep.maximum_speed = sqrt((float)2.0*pl_block->acceleration*intersect_distance+exit_speed_sqr);
						}          
					} 
    ...
    */
		...
		if (minimum_mm < (float)0.0) { minimum_mm = (float)0.0; }
		...
		do {
			switch (prep.ramp_type) 
			{
				case RAMP_ACCEL: 
					...
					mm_remaining -= time_var*(prep.current_speed + (float)0.5*speed_var);
    ...
    */
    ...
    // outputs the exact acceleration and velocity profiles as computed by the planner.
		...
		cycles = ceil( F_TIMER*inv_rate*60 ); // (cycles/step)    
		...
		{ 
			...
			if (mm_remaining > (float)0.0) // At end of forced-termination.
			{ 
