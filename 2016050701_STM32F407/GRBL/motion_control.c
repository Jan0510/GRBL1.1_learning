...
#endif
{
  ...
  do 
    ...
    if ( plan_check_full_buffer() )
		{ 
			protocol_auto_cycle_start();//Auto-cycle start置位，系统状态为cycle_start，当plan_buffer[]为满
		} 
    else 
		{ 
			break;
  ...
  #ifdef USE_LINE_NUMBERS
    plan_buffer_line(target, feed_rate, invert_feed_rate, line_number);  //规划一行G代码进入规划缓存区plan_buffer[]
  #else
...
#endif
{
  ...
  if (is_clockwise_arc) { // Correct atan2 output per direction
	...
	================================================================================*/
	if (angular_travel >= (float)-ARC_ANGULAR_TRAVEL_EPSILON) { angular_travel -= (float)(2*M_PI); }
  } else {
	/*==============================================================================
    if (angular_travel <= ARC_ANGULAR_TRAVEL_EPSILON) { angular_travel += 2*M_PI; }
	==============================================================================*/
  ...
  =======================================================================*/
  segments = (uint16_t)floor(fabs(0.5*(double)angular_travel*radius)/
                          sqrt(settings.arc_tolerance*(2*radius - settings.arc_tolerance)) );
  if (segments) { 
    ...
    */
    ...
    cos_T *= 0.5;
	========================================================*/
    cos_T = (float)(2.0 - (double)(theta_per_segment*theta_per_segment));
    sin_T = (float)((double)theta_per_segment*0.16666667*((double)cos_T + 4.0));
    cos_T *= (float)0.5;
