...
static void protocol_execute_line(char *line)
{
	...
	{
		// Parse and execute g-code block!　　　　　　分析执行Ｇ代码
...
void protocol_main_loop()
{
	...
	for (;;) 
	{
		...
		while((c = serial_read()) != SERIAL_NO_DATA)  //串口缓存区有数据执行
		{
			if ((c == '\n') || (c == '\r')) // End of line reached
			{
				line[char_counter] = 0; // Set string termination character.                                            //一行G代码结束为空字符
				protocol_execute_line(line); // Line is complete. Execute it!                                           //读取一行G代码指令完毕，开始执行
				comment = COMMENT_NONE;                                                                                 //清零
				char_counter = 0;                                                                                       //指令行数组指针清零
			} 
			else 
			{
		...
		//当串口接收缓存区没有将要处理和执行的数据时，这表明G代码已经装满planner buffer或者G代码接收完毕，
		//任何一种情况下auto_cycle_start置位。
