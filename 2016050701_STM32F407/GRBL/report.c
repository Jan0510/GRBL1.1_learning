...
{
	if (status_code == 0) // STATUS_OK 输出OK的原因：G代码行为空。
	{
...
void report_init_message()
{
	...
	//printPgmString(PSTR("\t 切割控制系统 ""['$' for help]\r\n"));
}
...
void report_grbl_help()
{
		...
		printPgmString(PSTR("$Nx=line (save startup block)\r\n"
							          "$C (check gcode mode)\r\n"
							          "$X (kill alarm lock)\r\n"
							          "$H (run homing cycle)\r\n"
							          "~ (cycle start)\r\n"
...
// 待system添加后测试	
void report_realtime_status()       //返回系统实时状态
{
	uint8_t idx;
