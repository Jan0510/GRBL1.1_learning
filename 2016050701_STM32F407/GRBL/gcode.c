...
uint8_t gc_execute_line(char *line) 
{
	...
	uint8_t axis_command;
	uint8_t axis_0, axis_1, axis_linear;
	uint8_t coord_select; // Tracks G10 P coordinate selection for execution
	...
	uint8_t axis_words; // XYZ tracking
	uint8_t ijk_words; // IJK tracking 

	// Initialize command and value words variables. Tracks words contained in this block.
	uint16_t command_words; // G and M command words. Also used for modal group violations.
	uint16_t value_words; // Value words. 
	
	uint8_t word_bit; // Bit-value for assigning tracking variables
	uint8_t char_counter;
	char letter;
	float value;        //存放XYZ目标值
	uint8_t int_value;  //浮点数的整数部分
	uint16_t mantissa;  //浮点数的小数部分
...
===========================================================*/
	...
	while (line[char_counter] != 0)// Loop until no more g-code words in line.                                     循环解析G代码，直到line[]中没有G代码关键字
	{    
		...
		int_value = trunc(value);                   //函数trunc直接舍掉数字的小数部分
		...
		case 'M':
			...
			switch(int_value) 
			{
				...
				case 10: case 11: case 12: case 13: case 14: case 15: case 16: case 17:
				  ...
				  switch(int_value)
					{
						case 10: SetLPOSwitch(); break;  //低压氧开关
						case 11: ResetLPOSwitch(); break;
						case 12: SetHPOSwitch(); break;  //高压氧开关
						case 13: ResetHPOSwitch(); break;
						case 14: SetGasSwitch(); break;  //燃气开关
						case 15: ResetGasSwitch(); break;
						case 16: SetAutoSwitch(); break; //自动点火开关
						case 17: ResetAutoSwitch(); break;
					}	
					break;
		...
		default: 
			...
			if ( bit(word_bit) & (bit(WORD_F)|bit(WORD_N)|bit(WORD_P)|bit(WORD_T)|bit(WORD_S)) ) 
			{
				...
				===============================================*/
				if (value < (float)0.0) { FAIL(STATUS_NEGATIVE_VALUE); } // [Word value cannot be negative]
			}
			value_words |= bit(word_bit); // Flag to indicate parameter assigned.
      
		}   
	} 
	// Parsing complete!　                                                                                                   　分析G代码完毕
	...
	*/  
	...
	// [20. Motion modes ]: 
	if (gc_block.modal.motion == MOTION_MODE_NONE) // [G80 Errors]: Axis word exist and are not used by a non-modal command.
	{   
	...
	else if ( axis_command == AXIS_COMMAND_MOTION_MODE ) 
	{
		...
		{      
			...
			===========================================================*/
			if (gc_block.values.f == (float)0.0) { FAIL(STATUS_GCODE_UNDEFINED_FEED_RATE); } // [Feed rate undefined]
            ...
            */  
            ...
            // Complete the operation by calculating the actual center of the arc
					...
					{ 
						...
						if (delta_r > 0.005) 
						=============================*/
						if (delta_r > (float)0.005) 
						{ 	
							/*======================================================
							if (delta_r > 0.5) { FAIL(STATUS_GCODE_INVALID_TARGET); } // [Arc definition error] > 0.5mm
							if (delta_r > (0.001*gc_block.values.r)) { FAIL(STATUS_GCODE_INVALID_TARGET); } // [Arc definition error] > 0.005mm AND 0.1% radius
							======================================================*/
							if (delta_r > (float)0.5) { FAIL(STATUS_GCODE_INVALID_TARGET); } // [Arc definition error] > 0.5mm
							if (delta_r > ((float)0.001*gc_block.values.r)) { FAIL(STATUS_GCODE_INVALID_TARGET); } // [Arc definition error] > 0.005mm AND 0.1% radius							
						}
					}
					break;
