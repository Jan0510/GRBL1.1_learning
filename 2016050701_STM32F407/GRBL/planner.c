...
#endif
{
  ...
  ======================================================================*/
  if (feed_rate < (float)MINIMUM_FEED_RATE) { feed_rate = (float)MINIMUM_FEED_RATE; } // Prevents step generation round-off condition.
  ...
  ===================================================*/
  inverse_millimeters = (float)(1.0/(double)block->millimeters);
  ...
  for (idx=0; idx<N_AXIS; idx++) {
    if (unit_vec[idx] != 0) {  // Avoid divide by zero.
	  ...
	  =================================================*/
	  inverse_unit_vec_value = fabs(1.0/(double)unit_vec[idx]); // Inverse to remove multiple float divides.
  ...
  if (block_buffer_head == block_buffer_tail) {
    ...
    */
	...
	============================================*/
	if (junction_cos_theta > (float)0.999999) {
	  ...
	  =========================================================*/
	  sin_theta_d2 = sqrt(0.5*(1.0-(double)junction_cos_theta)); // Trig half angle identity. Always positive.
	  ...
	  ==========================================================*/
	  block->max_junction_speed_sqr = max( MINIMUM_JUNCTION_SPEED*MINIMUM_JUNCTION_SPEED,
                                   (block->acceleration * settings.junction_deviation * sin_theta_d2)/(1.0-(double)sin_theta_d2) );
    }
  }
