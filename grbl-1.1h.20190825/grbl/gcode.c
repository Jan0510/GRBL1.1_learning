...
uint8_t gc_execute_line(char *line)
{
  ...
  // 从gc_state（当前指令状态）拷贝modal到gc_block，完成gc_block的初始化。
  // modal：模态命令，持续有效，因此需要拷贝给新的block
  // gc_block用来存新的GCode解释出来的内容。
  ...
  // 遍历line数组的字符，把GCode拆分塞进结构体gc_block
  while (line[char_counter] != 0) { // Loop until no more g-code words in line.
	// 导入GCode段（合法的格式：大写字母+float数字），G00.00、X00.00、Y00.00
    ...
    // math的库函数
    int_value = trunc(value);//向下取整
    mantissa =  round(100*(value - int_value)); // Compute mantissa(尾数) for Gxx.x commands.
    ...
    switch(letter) {
          ...
          case 80:
            ...
            if (int_value == 38){
              if (!((mantissa == 20) || (mantissa == 30) || (mantissa == 40) || (mantissa == 50))) {
				// 支持G38.2，38.3，38.4，38.5，但不支持其他38.X
      ...
      default:
        ...
        // 前后2段的字符重复。例如：X1.1X1.2，解释X1.1时，X会写入word_bit，word_bit会或入value_words。
        // 解释X1.2时，X会写入word_bit，并且value_words还是存在X位，以此来判断重复。
  ...
  */
  // 步骤3：检查block是否符合标准，若没错，则计算相应的数值传给执行队列。
  ...
  */
  ...
  // G93:反时限进给，在反时限进给速度模式下，F字表示移动应在[1除以F数]分钟内完成。
  // 例如，如果F值为2.0，则移动应在半分钟内完成。
  // 从G93切换到G94后，进给速度未定义，需要重新指定G94的速度。因为2者的速度换算方法不一样。
  // For jogging, 强制执行G94并检查所需的F字。G94是机床开机默认，每分钟进给率
  ...
  // 不支持刀具半径补偿。支持G40的目的只是为了在使用g代码程序头发送G40来设置默认模式时不会出错。
  ...
  // 不支持刀具长度补偿。G43.1表示对第1个道具进行长度补偿。G49表示0补偿。
  ...
  // modal_command：模态，意味着在后续的程序段中也生效，除非被同组的模态命令顶替。例如G01 G41 G42 G40以及F、S等
  // non_modal_command：非模态，意味着只在当前程序段生效，例如G4P2，停驻2秒。G10，对当前程序段设置坐标系。
      ...
      switch (gc_block.modal.motion) {
        ...
        case MOTION_MODE_CCW_ARC:
          ...
          if (value_words & bit(WORD_R)) { // Arc Radius Mode
		    // an example of a radius format command to mill an arc: G17 G2 x 10 y 15 r 20 z 5.
			// That means to make a clockwise (as viewed from the positive Z-axis) circular or helical arc whose
			// axis is parallel to the Z-axis, ending where X=10, Y=15, and Z=5, with a radius of 20
		  ...
		  // an example of a center format command to mill an arc: G17 G2 x 10 y 16 i 3 j 4 z 9
		  // If the current location has X=7, Y=7 at the outset, the center will be at X=10,Y=11.
            ...
            float target_r = hypot_f(x,y); // 目标点到圆心的半径

            // Compute arc radius for mc_arc. Defined from current location to center.
            gc_block.values.r = hypot_f(gc_block.values.ijk[axis_0], gc_block.values.ijk[axis_1]); // 起始点到圆心的半径
  ...
  */
  ...
  // 激光模式, setup laser power based on current and past parser conditions.
  ...
  // [21. Program flow ]:Program Stopping and Ending — M0, M1, M2, M30, M60
